...
class LaneNetDetector:
    ...
    def __init__(self, path_to_weights="../../../weights/yolopv2.pt"):
        ...
        self.model.to(
            self.device
        ).eval()  # Move model to device and set to evaluation mode
        ...
        self.Real_Height_SS = 0.75  # Height of stop sign in meters (not used currently)
        self.Brake_Distance = (
            5  # Distance at which to apply brakes (not used currently)
        )
        ...
        ####### (NP) added for forking
        self._fork_active = False
        self._fork_mask = None
        self._fork_timer = 0
        ...
        self.pub_waypoints = rospy.Publisher(
            "lane_detection/waypoints", Path, queue_size=1
        )
        self.pub_endgoal = rospy.Publisher(
            "LANE_DETECTION/endgoal", PoseStamped, queue_size=1
        )
    ...
    def img_callback(self, img):
        ...
        try:
            ...
            dimmed_gray = (img_gray * 0.5).astype(
                np.uint8
            )  # Reduce brightness of non-lane areas
            dimmed_gray[mask] = img_gray[
                mask
            ]  # Keep bright areas at original intensity
    ...
    def detect_lanes(self, seg, ll, img):
        ...
        ## added code here (NP)
        is_fork, ll_chosen = self._detect_fork_and_pick_right(ll_seg_mask)

        if is_fork:
            # engage the latch: remember this mask and hold for N frames
            self._fork_active = True
            self._fork_mask = chosen.copy()
            self._fork_timer = 20  # hold for the next 20 frames
            rospy.loginfo_throttle(
                2.0, "[LaneNet] Fork detected → latching RIGHT branch"
            )
        elif self._fork_active and self._fork_timer > 0:
            # still within the hold period: keep using the same branch mask
            chosen = self._fork_mask
            self._fork_timer -= 1
        else:
            # no active fork or hold expired: go back to normal
            self._fork_active = False
            chosen = ll_seg_mask

        # Generate waypoints from lane line mask
        waypoints, left_boundary = self.generate_waypoints(ll_chosen)
    ...
    def region_of_interest(self, img):
        ...
        polygon = np.array(
            [[(0, height), (0, 0), (width // 2, 0), (width // 2, height)]], np.int32
        )
    ...
    #             # OG Given # Calculate lane center by adding offset to left boundary
    #             # x_center = lb[0] + offset_pixels
    #             # y = lb[1]

    #             #Option 1
    #             x_left = lb[0]
    #             # Dynamic offset toward center
    #             offset_pixels = (width // 2) - x_left
    #             x_center = x_left + offset_pixels
    #             y = lb[1]
    ...
    # ---------------------------------------------------------------
    #  Fork / intersection detector  – always keep the right‑most lane
    # ---------------------------------------------------------------
    def _detect_fork_and_pick_right(self, ll_mask, area_px_thresh=6000):
        """
        Analyse the upper third of a lane-line mask.  If ≥ 2 substantial
        blobs are present (== a fork / multi-lane split), keep ONLY the
        *right-most* blob.  Returns:

            (fork_detected: bool, new_mask: np.uint8)

        new_mask is the binary lane-line mask after possibly dropping the
        non-rightmost branches.
        """
        h, w = ll_mask.shape

        # ── 1.  far‑field slice ‑‑ where the split first appears
        upper = ll_mask[: h // 3, :].copy()
        upper = cv2.dilate(upper, np.ones((4, 4), np.uint8), 1)  # close gaps

        # ── 2.  connected components
        num, labels, stats, centroids = cv2.connectedComponentsWithStats(
            upper, connectivity=8
        )

        # keep only blobs with decent area
        big = [
            (i, stats[i, cv2.CC_STAT_AREA], centroids[i])
            for i in range(1, num)
            if stats[i, cv2.CC_STAT_AREA] >= area_px_thresh
        ]

        if len(big) < 3:
            return False, ll_mask  # nothing to do

        # ── 3.  verify blobs are laterally separated
        big.sort(key=lambda x: x[2][0])  # sort by x‑centroid
        leftmost, *_, rightmost = big  # cheapest unpack

        if (rightmost[2][0] - leftmost[2][0]) < 0.15 * w:
            return False, ll_mask  # too close → ignore

        # ── 4.  build mask containing ONLY the right‑most blob
        right_label = rightmost[0]
        right_mask = (labels == right_label).astype(np.uint8)

        full = np.zeros_like(ll_mask, dtype=np.uint8)
        full[: h // 3, :] = right_mask  # far‑field
        full |= ll_mask[h // 3 :, :]  # keep near‑field
    ...
    def generate_waypoints(self, lane_mask):
        ...
        for lb, rb in zip(left, right):
            ...
            if lb is not None and rb is not None:
                ...
                lane_w = (
                    np.median(self.lane_width_history)
                    if self.lane_width_history
                    else self.estimated_lane_width_pixels
                )
        ...
        for point in boundary:
            if point is not None:
                if (
                    ...
                    or abs(point[0] - previous_point[0]) <= max_gap
                ):
    ...
    def draw_waypoints(self, img, waypoints, left_boundary):
        ...
        for lb in left_boundary:
            if lb is not None:
                cv2.circle(
                    img, (lb[0], lb[1]), radius=3, color=(255, 0, 0), thickness=-1
                )
